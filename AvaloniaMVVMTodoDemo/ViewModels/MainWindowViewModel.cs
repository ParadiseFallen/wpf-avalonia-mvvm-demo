...
namespace AvaloniaMVVMTodoDemo.ViewModels
{
    public class MainWindowViewModel : ViewModelBase
    {
        ...
        private IObservable<bool> editEnable;
        public MainWindowViewModel(Database db)
        {
            Content = List = new TodoListViewModel(db.GetItems());
            deleteEnable = this.List.CheckedItems.WhenAnyValue(x => x.Count, x => x != 0);
            editEnable = this.List.CheckedItems.WhenAnyValue(x => x.Count, x => x == 1);
        ...
        public ReactiveCommand<Unit, Unit> Edit
        {
            get => ReactiveCommand.Create(
              ()=>{
                var vm = new AddItemViewModel(this.List.CheckedItems.FirstOrDefault());
                Observable.Merge(
                vm.Ok,
                vm.Cancel.Select(_ => (TodoItem)null))
                .Take(1) // Обработчик результата диалога
                ...
                Content = vm;
                  ...
                  this.List.CheckedItems.WhenAnyValue(x=>x.Count,x=>x==1)
              );
